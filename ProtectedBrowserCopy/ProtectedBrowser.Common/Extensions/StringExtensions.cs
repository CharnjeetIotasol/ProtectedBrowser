using ProtectedBrowser.Common.Utility;
using System;
...
namespace ProtectedBrowser.Common.Extensions
{
    ...
    public static class StringExtensions
    {
        ...
        /// <param name="defaultRegex"></param>
        /// <returns></returns>
        public static string RemoveAllSpecialChars(this string str, string defaultRegex = "[^a-zA-Z0-9 ]+")
        {
            if (defaultRegex.IsNullOrEmpty() || str.IsNullOrEmpty()) return null;

            return Regex.Replace(str, "[^a-zA-Z0-9 ]+", "", RegexOptions.Compiled);
        }
        public static bool IsCorrectMobileNumber(this string strNumber)
        {
            Regex mobilePattern = new Regex(@"^(?:(?:\+|0{0,2})91(\s*[\ -]\s*)?|[0]?)?[789]\d{9}|(\d[ -]?){10}\d$");
